...
def create_learning_progression_plot(progression):
    ...
    fig.suptitle('WebGuard Learning Progression Analysis', fontsize=16, fontweight='bold')
...
def create_behavioral_analysis(df):
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle('WebGuard Behavioral Analysis', fontsize=16, fontweight='bold')
...
def create_feature_validation_dashboard(df, progression):
    ...
    fig.suptitle('WebGuard Feature Validation Dashboard', fontsize=20, fontweight='bold')
